...
// ---------------------------------------------------------------------------------
// Toggle API additions
// ---------------------------------------------------------------------------------
//@input bool startEnabled = true {"label":"Start Enabled"}

var _oceanEnabled = true;            // runtime flag
var _owned = [];                     // every SceneObject created by this script
var fluid2DComp = null;              // Fluid2D component reference (if used)
var fluid2DTouchSO = null;           // Touch helper SO (if created)

function setOceanEnabled(on) {
    _oceanEnabled = !!on;
    for (var i = 0; i < _owned.length; i++) {
        var so = _owned[i];
        if (so && so.isOfType) { so.enabled = _oceanEnabled; }
    }
...
function initialize(){
    ...
    if ( script.enableTouchInput ){
        // Generate fluid 2D
        fluid2DComp = script.sceneObject.createComponent( script.fluid2DSim );
    }
...
function createOceanPlane(){
    ...
    if ( script.enableTouchInput ){
        ...
        fluid2DTouchSO = global.scene.createSceneObject("fluid2DTouchSO");
        _owned.push(fluid2DTouchSO);
    ...
    if( script.InfiniteOcean ){
        ...
        event.bind(function(){
            // Pause work while disabled
            if (!_oceanEnabled) { return; }

            // Attach ocean plane onto the camera position to create infinite plane
            ...
            if ( script.enableTouchInput && fluid2DTouchSO ){
...
function bodyTrackingDetect(pass){
    ...
    _owned.push(bodyTrackingObject);
...
function setOceanPropertyTex(){
    ...
    _owned.push(oceanPropertyObject);
