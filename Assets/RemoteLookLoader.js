...
// Lock PNG (optional). Drag the PURPLE Image component of your lock icon here.
// If left empty, the script will try to find an Image on this node, then in children.
//@input Component.Image lockImage
...
function hashNames(arr){ return arr.join("|"); }
...
// --- Lock image helpers (component-only; won't affect siblings like the carousel) ---
function resolveLockImage(){
    if (script.lockImage) return script.lockImage;
    var so = script.getSceneObject();
    if (!so) return null;
    var img = so.getFirstComponent("Component.Image");
    if (img) return img;
    var imgs = so.getComponentsInChildren("Component.Image");
    return (imgs && imgs.length) ? imgs[0] : null;
}
function lockShow(){ var img = resolveLockImage(); if (img) img.enabled = true; }
function lockHide(){ var img = resolveLockImage(); if (img) img.enabled = false; }
// ------------------------------------------------------------------------------------
...
function selectIndex(i){
    ...
    if (!unlocked){
        ...
        lockShow();                 // show lock PNG when locked
        currentIndex = i;
        return;
    }
    ...
    setHeadmorphEnabled(true);
    applyModel(i);
    lockHide();                    // hide lock PNG when unlocked
    currentIndex = i;
}
...
script.createEvent("OnStartEvent").bind(function(){
    ...
    // Start with lock shown until a selection proves unlocked (optional)
    lockShow();
